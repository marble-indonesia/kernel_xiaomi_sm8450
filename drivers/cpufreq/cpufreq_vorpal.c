 ...
 * burst floor is always below it. Sustained latch (>=LIFT_PCT) lifts to 88%
 * for long foreground work, releasing below DROP_PCT.
 *
 * Base caps (70/68) hold background work at lower voltage for MTK daily heat;
 * boost caps are untouched, so touch/scroll stays as responsive. DROP 40 (raw
 * ~24% after the 1.25x skew) releases the sustained latch promptly once load
 * eases, back to the cool base cap instead of holding 88%.
 */

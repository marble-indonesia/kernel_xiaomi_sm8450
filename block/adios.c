...
// Global latency window. UFS/eMMC tuning; no rotational variant -- this tree
// has no queue-rotational API, so the guarded seek paths below are dormant.
static u64 default_global_latency_window            = 12000000ULL;
...
// Latency targets (async only; sync writes bypass via Tier-2). UFS/eMMC values.
static u64 default_latency_target[ADIOS_OPTYPES] = {
	[ADIOS_READ]    =     2ULL * NSEC_PER_MSEC,
	[ADIOS_WRITE]   =   250ULL * NSEC_PER_MSEC,
...
static void adios_limit_depth(unsigned int opf, struct blk_mq_alloc_data *data) {
	...
	/* async_depth is sized from the tag map in adios_depth_updated(); the
	 * sbitmap clamps shallow_depth per word. */
...
static void adios_depth_updated(struct blk_mq_hw_ctx *hctx) {
	...
	/* Cap async allocation at 3/4 of one sbitmap word. UFS/eMMC queue
	 * management; the sbitmap clamps shallow_depth per word. */
...
static void insert_to_prio_queue(struct adios_data *ad,
	...
	/* Unmanaged requests are not counted: adios_completed_request() bails
	 * out before subtracting, so adding here would leak the count. */
	if (likely(rd)) {
		union adios_in_flight_rqs ifr = {
			.count          = 1,
			.total_pred_lat = min_t(u64, rd->pred_lat,
						(1ULL << 48) - 1),
		};
...
static void adios_prepare_request(struct request *rq) {
	...
	/* GFP_ATOMIC: the pool reserve is one queue depth, so a shared-tag or
	 * multi-hctx queue can outrun it and the slab fallback can fail. A
	 * request with no latency state is still dispatchable -- leave priv[0]
	 * NULL, send it FIFO, and let the completion path skip it. */
...
static int adios_init_sched(struct request_queue *q, struct elevator_type *e) {
	...
	/* This tree exposes no queue-rotational API and the target storage is
	 * UFS/eMMC, so the seek-weighting paths below stay dormant. */
	ad->is_rotational = 0;
	...
	/* Deferrable: the model folds deltas already accumulated in the per-CPU
	 * buckets, so running it late changes nothing -- but a non-deferrable
	 * timer pulls a CPU out of idle 100ms after every I/O burst, which is
	 * exactly the screen-off case this scheduler should not be paying for. */
